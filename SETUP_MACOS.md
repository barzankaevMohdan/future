...
> 💡 Для каждой камеры нужен отдельный экземпляр Recognition Service с разными `--camera-id` и `--camera-source`

Сервис доступен на: `http://localhost:5001`
...
1. Войдите в админ-панель как Админ компании
2. Перейдите в раздел **Камеры**
3. Нажмите **Добавить камеру**
4. Заполните форму:
   - **Название:** Например, "Вход в офис"
   - **RTSP URL:** `rtsp://username:password@192.168.1.10:554/stream`
   - **Статус:** Активна
5. Сохраните

После добавления камеры:
- Видеопоток доступен: `http://localhost:4000/streams/{camera_id}.mjpg`
- Запустите Recognition Service для этой камеры
